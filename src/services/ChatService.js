...
class ChatService {
  ...
  async getCachedConversations() {
    try {
      ...
      // If cache is older than retention period, consider it stale
          ...
          snapshot => {
            try {
              ...
              const filteredConversations = conversations.filter(
                conv => !(conv.deleted && conv.deleted[userId])
              );
              
              // Cache conversations data
              this.cacheConversations(filteredConversations);
  ...
  async getOrCreateConversation(otherUserId) {
    try {
      ...
      if (existingConversation) {
        const convData = existingConversation.data();
        
        // Reset deletion status if this user previously deleted the conversation
        if (convData.deleted && convData.deleted[userId]) {
          await firestore()
            .collection('conversations')
        ...
        return {
          ...
          ...convData,
          lastMessageTimestamp: convData.lastMessageTimestamp?.toDate() || null,
          deleted: {
            ...(convData.deleted || {}),
            [userId]: false
          }
