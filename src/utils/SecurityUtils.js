...
export default securityUtils; from 'react-native';
...
// Note: We're using a direct import because CryptoJS may not be available
let CryptoJS;
try {
...
class SecurityUtils {
  ...
  async getSecureKey() {
    try {
      ...
      if (storageKey) {
        ...
        if (Platform.OS === 'ios' || Platform.OS === 'android') {
          try {
            await Keychain.setGenericPassword('deviceEncryptionKey', storageKey, {
              service: 'deviceEncryptionKey',
  ...
  async saveSecureKey(key) {
    try {
      ...
      if (Platform.OS === 'ios' || Platform.OS === 'android') {
        try {
          await Keychain.setGenericPassword('deviceEncryptionKey', key, {
            service: 'deviceEncryptionKey',
            accessible: Keychain.ACCESSIBLE.WHEN_UNLOCKED_THIS_DEVICE_ONLY
          });
          return true;
